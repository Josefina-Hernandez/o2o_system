...
class CreateMColorTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        Schema::create(config('const_db_tostem.db.m_color.nametable'), function (Blueprint $table) {
            $table->increments(config('const_db_tostem.db.m_color.column.M_COLOR_ID')); 
            $table->string(config('const_db_tostem.db.m_color.column.COKOR_CODE_PRICE'))->nullable();
            $table->integer(config('const_db_tostem.db.m_color.column.SORT_ORDER'))->nullable();
    ...
    {
        Schema::dropIfExists(config('const_db_tostem.db.m_color.nametable'));
    }
}
