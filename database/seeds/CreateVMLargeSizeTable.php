...
class CreateVMLargeSizeTable extends Seeder
{
    /**
    ...
    public function run()
    {
		...
		$sql_insert_tmp = '
			...
			WHERE CONCAT(re.product_id, re.m_model_id) not in (SELECT CONCAT(door.product_id,door.m_model_id) FROM m_large_size as door WHERE door.del_flg = 0)
			ORDER BY product_id, m_model_id';
		
		 $sql_insert = 'CREATE OR REPLACE TABLE v_m_large_size
		(
			SELECT *
			FROM tmp_table
		)';
