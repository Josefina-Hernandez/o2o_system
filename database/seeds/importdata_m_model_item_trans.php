...
class importdata_m_model_item_trans extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $table = 'm_model_item_trans';	
        if ($import->truncate_db($table)) {
            echo "$table TRUNCATE OK!" .PHP_EOL;
        }
        
        $txt_file_lang_en =  storage_path() . '/import_data/m_model_item_trans.txt';
        $this->_ImportDataToDB($import,$txt_file_lang_en,$table);// lang EN
        
        $txt_file_lang_th = storage_path() . '/import_data/m_model_item_trans_THAI.txt';
