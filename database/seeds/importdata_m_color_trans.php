...
class importdata_m_color_trans extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $table = 'm_color_trans';
        //$column_db = ['m_color_id', 'm_lang_id', 'color_name', 'del_flg'];

        if ($import->truncate_db($table)) {
            echo "$table TRUNCATE OK!" .PHP_EOL;
        }
        
        $txt_file_lang_en = storage_path() . '/import_data/m_color_trans.txt';
        $this->_ImportDataToDB($import,$txt_file_lang_en,$table);// lang EN
        
        $txt_file_lang_th = storage_path() . '/import_data/m_color_trans_THAI.txt';
