...
class CreateVProductPriceReferTable extends Seeder
{
    /**
    ...
    {
    	$table = 'v_product_price_refer';
    	$sql = "
    		CREATE OR REPLACE TABLE v_product_price_refer
			(
				SELECT DISTINCT 
					pr.*
				...
				INNER JOIN m_selling_code AS sc
					ON
						...
						AND sc.del_flg = 0
				INNER JOIN product_selling_code_price AS pr
					ON
						pr.design = sc.selling_code
				WHERE ms.del_flg = 0
				ORDER BY sc.m_selling_code_id, sc.product_id, ms.m_model_id, pr.product_selling_code_price_id
			)
    	";
        
        $sql_1 = "
        	--
			-- Indexes for table `v_product_price_refer`
			--
			ALTER TABLE `v_product_price_refer`
			  ADD PRIMARY KEY (`product_selling_code_price_id`),
