...
class ImportDatactg extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $txt_file = storage_path() . '/import_data/ctg.txt';
        $table = 'ctg';
       // $column_db = ['ctg_id', 'ctg_type', 'parent_ctg_id', 'del_flg', 'sort_order'];
        ...
        if (file_exists($txt_file) === true) {
            ...
            foreach ($data_insert as $k => $data) {
                if ($data['parent_ctg_id'] == '') {
                    $data_insert[$k]['parent_ctg_id'] = NULL;
                }
                if ($data['prod_parent_ctg_id'] == '') {
                    $data_insert[$k]['prod_parent_ctg_id'] = NULL;
                }
            }
