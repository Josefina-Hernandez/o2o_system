...
class importdata_m_color_ctg_prod extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $txt_file = storage_path() . '/import_data/m_color_ctg_prod.txt';
        $table = 'm_color_ctg_prod';
        ...
        if (file_exists($txt_file) === true) {
            ...
            foreach ($data_insert as $k => $data) {
                foreach ($column_db as $value) {
                    ...
                    if($value == 'del_flg' && $data['del_flg'] == '')
                    {
                        $data_insert[$k][$value] = 0;
                    }
                }
            }
