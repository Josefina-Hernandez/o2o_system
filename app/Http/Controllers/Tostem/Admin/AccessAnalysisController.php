<?php
namespace App\Http\Controllers\Tostem\Admin;

use App\Http\Controllers\Controller;
use App\Models\Tostem\{
              AccessAnalysis
};
use Illuminate\Http\Request;
use App\Services\ExcelService;


Class AccessAnalysisController extends Controller
{

    public function __construct()
    {
         $this->middleware('auth');
    }

    public function index (Request $request) {
         
       
       $_all_historys = AccessAnalysis::Searchdata([]);
    
        if ($request->ajax()) {
               if($_all_historys->count() > 0){
                         $html = view('tostem.admin.accessanalysis.module-content.module-listhistory',compact('_all_historys'))->render();
                         $return['html'] = $html;
                         return $return;
               }
               
        }
       
       
       return view('tostem.admin.accessanalysis.index',compact('_all_historys'));
       
    }
    
    public function searchdata(Request $request) {
         
          $regEx = '/^\d{4}\/\d{2}\/\d{2}$/';

          $param = $request->all();
          $param_where = [];
        
          if(!empty($param['time_strart'])){
               
               $check_date = preg_match($regEx,$param['time_strart']);
               if(!$check_date){
                    $return['status'] = 'error_time';
                    return $return;
               }
               
               $param_where['time_strart'] = $param['time_strart'];
               
          }
          if(!empty($param['time_end'])){
               
               $check_date = preg_match($regEx,$param['time_end']);
               if(!$check_date){
                    $return['status'] = 'error_time';
                    return $return;
               }
               
               $param_where['time_end'] = $param['time_end'];
               
          }
          if(!empty($param['page'])){
               
               $param_where['page'] = $param['page'];
               
          }
          
          $_all_historys = AccessAnalysis::Searchdata($param_where);
          
          $_all_historys->appends($param)->links();
        
            if ($request->ajax()) {
                         $html = view('tostem.admin.accessanalysis.module-content.module-listhistory',compact('_all_historys'))->render();
                         $return['html'] = $html;
                         return $return;
            }
            return view('tostem.admin.accessanalysis.index',compact('_all_historys'));
      
    }
    
    
    
     public function downloadfile(Request $request){
          
          $regEx = '/^\d{4}\/\d{2}\/\d{2}$/';
          
          $param = $request->all();
          
          $param_where = [];
          
        
          if(!empty($param['time_strart'])){
               
               $check_date = preg_match($regEx,$param['time_strart']);
               if(!$check_date){
                    $return['status'] = 'error_time';
                    return $return;
               }
               
               $param_where['time_strart'] = $param['time_strart'];
               
          }
          if(!empty($param['time_end'])){
               
               $check_date = preg_match($regEx,$param['time_end']);
               if(!$check_date){
                    $return['status'] = 'error_time';
                    return $return;
               }
               
               $param_where['time_end'] = $param['time_end'];
               
          }
           if(!empty($param['page'])){
               
               $param_where['page'] = $param['page'];
               
          }
          
         
          $name_result = 'access_analysis_result_'.date('yymdhms').'.csv';
          
          
          $elo = new AccessAnalysis();
          
          $elo->setTypeSelect(1);
          
          $elo->_exportDataAdmin($param_where);
          
          
          $csv = new ExcelService('',$elo);
          
          $csv->setNamefile($name_result);
          
          $csv->setHeaderExcel(array_keys($elo->getColumnExport()));
          $csv->setAutoIncrease('stt');
          $csv->_exportCsvToBrowser();
  
    }
    public function checkAuth(){
         
          $return['status'] = 'OK';
          return $return; 
          
    }


}