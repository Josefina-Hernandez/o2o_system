...
/*!*****************************************************************************!*\
  !*** ./resources/assets/tostem/admin/quotationresult/js/quotationresult.js ***!
  \*****************************************************************************/
var pa = new params();
var p = new pages();
$(function () {
  ...
  $('#parentdata').scroll(function () {
    //detect page scroll
    var _element = document.getElementById("parentdata");

    if (_element.scrollTop + _element.offsetHeight >= _element.scrollHeight - 31000) {
...
$(document).on("click", ".clear_input_search", function () {
  $('.start-date').val('');
  $('.end-date').val('');
  $('.quotaition-no').val(''); //pa.param = _set_param(null, null, null);
});
...
$(document).on("click", ".download-file-csv", function () {
  ...
  var $_time_end = pa.param._time_end; //pa.param = _set_param($_quotaition_no, $_time_strart, $_time_end);
...
function load_view_data(page) {
  jQuery.ajax({
    ...
    success: function success(data) {
      ...
      if (data.status == 'full') {
        $('#type-data').val('3'); //$('#list-data').append(_template_end());
...
function _template_viewload() {
  var _temp = "<div id='loading-data' style='text-align: center;'>" + " <img src='" + config.image._loading_data + "' width='70px'>" + "</div>";

  return _temp;
}

function _template_end() {
  var _temp = "<div id='loading-data' style='text-align: center;'>" + "<h3>End</h3>" + "</div>";
...
/******/ })()
;
//# sourceMappingURL=quotationresult.js.map